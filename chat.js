...
function displayMessage(message) {
    ...
    if (message.text && message.text.startsWith('>')) {
        ...
        const quotePart = parts[0].substring(1).trim(); // Remove the '>'
        const replyPart = parts[1] || '';
        
        messageContent += `
            <div class="message-quote">
                <i class="fas fa-reply"></i>
                <div class="quote-content">${escapeHtml(quotePart)}</div>
            </div>
        `;
        
        if (replyPart) {
            messageContent += `<div class="message-text">${escapeHtml(replyPart)}</div>`;
        }
