// chat.js - Enhanced Chat Functionality (FIXED VERSION)
...
const ChatModule = (function() {
    ...
    // Display a message in the chat (optimized)
    function displayMessage(message) {
        ...
        // Store in appState.messages for future reference (limit to 100)
        ...
        if (shouldScroll && !scrollPositionLock) {
    ...
    function saveScrollPosition() {
        ...
        sessionStorage.setItem('chatScrollPosition', elements.chatMessages.scrollTop);
    }
    ...
    function restoreScrollPosition() {
        ...
        if (savedPosition) {
            elements.chatMessages.scrollTop = parseInt(savedPosition);
        }
    }

    // Helper function to get reply quote HTML (optimized)
    ...
    // FIXED: Open reply modal - better mobile handling
    function openReplyModal(messageId, senderName, messageText) {
        ...
        if (isProcessingReply) {
            console.log('Already processing reply, ignoring');
            return;
        }
        ...
        scrollPositionLock = true;
        
        // On mobile, add class to body to prevent background scrolling
        document.body.classList.add('modal-open');
        
        // Focus on the input with delay for mobile keyboard
        setTimeout(() => {
            if (elements.replyInput) {
                elements.replyInput.focus();
                
                // On mobile, ensure keyboard opens and scroll to input
                if (window.innerWidth <= 768) {
                    elements.replyInput.click();
                    
                    // Scroll to the reply input on mobile
                    setTimeout(() => {
                        if (elements.replyInput) {
        ...
        // Unlock scroll after modal is closed
        const closeModal = () => {
            scrollPositionLock = false;
        };
        
        // Add one-time close handler
        const originalClose = elements.closeReplyModal.onclick;
        elements.closeReplyModal.onclick = () => {
            closeModal();
            if (originalClose) originalClose();
        };
    }
    
    // FIXED: Send reply - prevents double sends and fixes keyboard issues
    async function sendReply() {
        console.log('🟢 sendReply called at:', new Date().toISOString());
        
        // Prevent multiple simultaneous replies
        if (isProcessingReply) {
        ...
        try {
            // Call sendMessage with a small delay to ensure modal is fully closed
    ...
    // FIXED: Setup event listeners with proper mobile handling
    function setupEventListeners() {
        // Handle reply button clicks - using event delegation for better performance
        ...
        // Add both click and touch events with passive false for better touch handling
        document.addEventListener('click', handleReplyClick);
        document.addEventListener('touchstart', handleReplyClick, { passive: false });

        // FIXED: Send reply button handler - prevents double sends
        if (elements.sendReplyBtn) {
            ...
            const handleSendReply = async function(e) {
                e.preventDefault();
                e.stopPropagation();
        ...
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape' && elements.replyModal && elements.replyModal.style.display === 'flex') {
                elements.replyModal.style.display = 'none';
                document.body.classList.remove('modal-open');
                if (appState) appState.replyingTo = null;
                scrollPositionLock = false;
