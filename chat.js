...
const ChatModule = (function() {
    ...
    const openReplyModal = (messageId, senderName, messageText) => {
        ...
        if (imageUrl) {
            const imgHtml = `<div style="margin-top:10px;display:flex;align-items:center;gap:10px;background:rgba(0,0,0,0.05);padding:8px;border-radius:8px;"><img src="${imageUrl}" style="max-width:60px;max-height:60px;border-radius:8px;object-fit:cover;"><span><i class="fas fa-image"></i> Image attached</span></div>`;
            displayText = imgHtml + (displayText ? `<div style="margin-top:8px;">${escapeHtml(displayText)}</div>` : '');
        } else {
    ...
    const sendReply = async () => {
        ...
        if (!replyData?.messageId) {
            console.log('No reply data found');
            return;
        }
...
console.log('Chat.js loaded - reply function fixed');
