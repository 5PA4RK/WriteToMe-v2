...
// Reply to message
window.replyToMessage = function(messageId, senderName = null, messageText = null) {
    ...
    replyIndicator.innerHTML = `
        ...
        <button class="btn btn-small btn-secondary" onclick="cancelReply()">
...
window.addEmojiToMessage = async function(messageId, emoji) {
    try {
        ...
        // Don't alert for emoji errors - they're non-critical
        return 'error';
    }
};

// Show emoji picker for a specific message
window.showMessageEmojiPicker = function(messageId, event) {
    ...
    quickEmojis.forEach(emoji => {
        ...
        emojiBtn.onclick = (e) => {
            e.stopPropagation();
            addEmojiToMessage(messageId, emoji);
            picker.remove();
        };
    ...
    moreBtn.onclick = (e) => {
        e.stopPropagation();
        showFullEmojiPicker(messageId, event);
        picker.remove();
    };
...
function showFullEmojiPicker(messageId, event) {
    if (event) event.stopPropagation();
        ...
        <div class="emoji-picker-content">
            ...
            <div class="emoji-grid">
                ${appState.emojis.map(emoji => `
                    <button class="emoji-option" onclick="addEmojiToMessage('${messageId}', '${emoji}'); this.closest('.emoji-picker-modal').remove()">
                        ${emoji}
                    </button>
                `).join('')}
            </div>
    ...
    try {
        ...
        if (imageFile) {
            ...
            reader.onload = async function(e) {
                ...
                await sendMessageToDB(messageText, imageUrl, replyToId);
            ...
            await sendMessageToDB(messageText, null, replyToId);
...
// Updated sendMessageToDB function with reply support
...
function displayMessage(message) {
    ...
    if (message.reply_to_id) {
        replyInfo = `
            <div class="reply-indicator-message" onclick="scrollToMessage('${message.reply_to_id}')">
                <i class="fas fa-reply"></i>
        ...
        emojiReactions = `
            <div class="emoji-reactions">
                ${reactions.map(([emoji, count]) => `
                    <span class="emoji-reaction" onclick="addEmojiToMessage('${message.id}', '${emoji}')">
                        ${emoji} ${count}
                    </span>
                `).join('')}
            </div>
    ...
    messageDiv.innerHTML = `
        ...
        <div class="message-actions">
            ${canEditDelete ? `
                <button class="message-action-btn" onclick="editMessage('${message.id}')" title="Edit message">
                    <i class="fas fa-edit"></i>
                </button>
                <button class="message-action-btn" onclick="deleteMessage('${message.id}')" title="Delete message">
                    <i class="fas fa-trash"></i>
                </button>
            ` : ''}
            <button class="message-action-btn" onclick="replyToMessage('${message.id}')" title="Reply to message">
                <i class="fas fa-reply"></i>
            </button>
            <button class="message-action-btn" onclick="showMessageEmojiPicker('${message.id}', event)" title="Add reaction">
...
// ============================================
// UPDATE LOAD CHAT HISTORY FUNCTION
// ============================================
...
// ============================================
// SETUP EMOJI REACTIONS SUBSCRIPTION
// ============================================

function setupEmojiReactionsSubscription() {
                            ...
                            if (reactionsContainer) {
                                if (message.emoji_reactions && Object.keys(message.emoji_reactions).length > 0) {
                                    ...
                                    reactionsContainer.innerHTML = reactions
                                        .map(([emoji, count]) => `
                                            <span class="emoji-reaction" onclick="addEmojiToMessage('${messageId}', '${emoji}')">
                                                ${emoji} ${count}
                                            </span>
                                        `).join('');
...
// Update setupRealtimeSubscriptions to include emoji reactions
// In setupRealtimeSubscriptions function, add this line at the end:
// setupEmojiReactionsSubscription();
...
function setupRealtimeSubscriptions() {
    ...
    // Create NEW subscription with simpler configuration
    appState.realtimeSubscription = supabaseClient
        .channel('public:messages')  // Simpler channel name
        .on(
            'postgres_changes',
            {
                event: 'INSERT',
...
// In the loadChatSessions function, update the card HTML generation:
// Replace with guest-focused information only:
